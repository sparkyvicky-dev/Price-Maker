...
import type { FooterTemplate, HeaderData, Product } from '../types';
...
const SECTION_BRAND_MAP: Record<string, string> = {
  vivo: 'Vivo',
  iqoo: 'IQOO',
  nothing: 'Nothing',
  moto: 'Motorola',
...
export type ExcelParseMeta = {
  format: 'generic' | 'sparky-stock' | 'stock-summary' | 'pakkabill';
...
export function parseExcelBuffer(buffer: ArrayBuffer): ExcelParseResult {
  ...
  const meta: ExcelParseMeta = { format: 'generic', missingPrices: 0 };
...
function detectFormat(rows: string[][]): ExcelParseMeta['format'] {
...
function isStockSummaryHeader(row: string[]): boolean {
...
function isSparkySectionHeader(row: string[]): boolean {
...
function parseSparkyStockReport(rows: string[][]): { items: Product[]; missingPrices: number } {
  const products: Product[] = [];
  let currentBrand = '';
...
function parseStockSummaryReport(rows: string[][]): { items: Product[]; missingPrices: number } {
  let headerIdx = -1;
  let headers: string[] = [];
...
function parsePakkaBillExport(rows: string[][]): Product[] {
  let headerIdx = -1;
  let headers: string[] = [];
...
function parseEmbeddedProductName(
  productName: string,
  sectionBrand: string,
): { brand: string; model: string; ram: string; storage: string } {
...
function normalizeSectionBrand(raw: string): string {
...
function stripBrandFromModel(name: string, brand: string): string {
  const detected = detectBrand(name.split(' ')[0], name);
  return detected !== 'Other' ? detected : brand;
}

function extractSparkyPrice(row: string[], hasPriceColumn: boolean): number {
...
function extractPriceFromRow(row: string[]): number {
...
function parseGenericSheet(rows: string[][], sheetName: string): Product[] {
  let headerRowIdx = 0;
...
function normalizeSpec(val: string): string {
  if (!val) return '';
...
function deduplicateProducts(products: Product[]): Product[] {
  ...
  for (const p of products) {
    ...
    const normalized: Product = { ...p, model };
...
export type WorkbookHeader = Partial<HeaderData> & {
  footer?: FooterTemplate;
};

export function productsToWorkbook(products: Product[], header: WorkbookHeader = {}): XLSX.WorkBook {
